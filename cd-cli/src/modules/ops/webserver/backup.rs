...
use crate::config::{CeresConfig as Config, Profile, Provider};
use crate::modules::*;
use crate::output::OutputType;
use crate::provider::filter;
use crate::provider::{DescribeInstances, InstanceDescriptor};
...
fn do_call(args: &ArgMatches, run_config: &RunConfig, config: &Config) -> Result<()> {
    ...
    let commands: Result<Vec<_>> = COMMANDS
        .iter()
        .map(|c| {
            let command_args: Vec<_> = c.split(' ').collect();
            ssh::build_ssh_command_to_instances(
                &instances,
                public_ip,
                profile.ssh_user.as_ref(),
                &ssh_opts,
    ...
    let results: Result<Vec<_>> = commands
        .into_iter()
        .map(|cs| run::run(cs, progress_bar).chain_err(|| ErrorKind::ModuleFailed(NAME.to_owned())))
        .collect();
...
fn find_instances(profile: &Profile) -> Result<Vec<InstanceDescriptor>> {
    let Provider::Aws(provider) = profile
        .provider
        .as_ref()
    ...
    let filter = filter::FilterBuilder::new()
        ...
        .build()
        .chain_err(|| ErrorKind::ModuleFailed(NAME.to_owned()))?;
    let webservers = all.into_iter().filter(|i| filter.filter(i)).collect();
