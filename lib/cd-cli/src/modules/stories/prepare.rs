...
fn do_call(args: &ArgMatches, run_config: &RunConfig, config: &Config) -> Result<()> {
    ...
    let story: Story = core.run(f).chain_err(|| ErrorKind::FailedToQueryPivotalApi)?;
    debug!("{:#?}", story);
    ...
    let result: Result<Vec<_>> = TASKS
        ...
        .map(|(pos, x)| {
            ...
            core.run(f).chain_err(|| ErrorKind::FailedToQueryPivotalApi)
        })
        .collect();
    let result = result?;
    debug!("{:#?}", result);
    ...
    let description = format!(
        "{}\n\n{}\n",
        story.description.unwrap_or_else(|| "".to_string()),
        NO_RISK_ASSESSMET
    );
    let f = set_description(&client, project_id, story_id, &token, &description);
    let result = core.run(f).chain_err(|| ErrorKind::FailedToQueryPivotalApi)?;
    debug!("{:#?}", result);
