...
use crate::config::CeresConfig as Config;
use crate::modules::stories::errors::*;
use crate::modules::{Error as ModuleError, ErrorKind as ModuleErrorKind, Module, Result as ModuleResult};
use crate::output::stories::OutputType;
use crate::output::stories::{JsonOutputStory, MarkDownOutputStory, OutputStory};
use crate::run_config::RunConfig;
use pivotal_api::{get_project_members, get_story, ProjectMember, Story};
...
fn do_call(args: &ArgMatches, run_config: &RunConfig, config: &Config) -> Result<()> {
    ...
    info!("Quering story");
    let mut core = tokio_core::reactor::Core::new().chain_err(|| ErrorKind::FailedToQueryPivotalApi)?;
    let client = ReqwestClient::new();

    let work = get_story(&client, project_id, story_id, &token);
    let story = core.run(work).chain_err(|| ErrorKind::FailedToQueryPivotalApi)?;
    debug!("{:#?}", story);

    let work = get_project_members(&client, project_id, &token);
    let members = core.run(work).chain_err(|| ErrorKind::FailedToQueryPivotalApi)?;
    debug!("{:#?}", members);
...
fn output_story(args: &ArgMatches, _: &RunConfig, _: &Config, story: &Story, members: &[ProjectMember]) -> Result<()> {
