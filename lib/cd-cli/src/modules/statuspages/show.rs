...
use crate::modules::statuspages::errors::*;
use crate::modules::statuspages::{PageStatus, PageStatusResult};
use crate::modules::{Error as ModuleError, ErrorKind as ModuleErrorKind, Module, Result as ModuleResult};
use crate::output::statuspages::*;
...
fn do_call(args: &ArgMatches, _: &RunConfig, config: &Config) -> Result<()> {
    ...
    info!("Quering status");
    let mut core = tokio_core::reactor::Core::new().chain_err(|| ErrorKind::FailedToQueryStatusPage)?;
    let client = ReqwestClient::new();

    let queries = status_pages
        .iter()
        .map(|(name, status_page)| query_page_status(&client, name.to_string(), &status_page.id));
...
fn query_page_status(
    client: &ReqwestClient,
    name: String,
    id: &str,
) -> impl Future<Item = PageStatusResult, Error = Error> {
...
fn output_page_status(output_type: OutputType, status: &[PageStatusResult]) -> Result<()> {
    ...
    match output_type {
        OutputType::Human => {
            ...
            output.output(&mut stdout, status).chain_err(|| ErrorKind::FailedOutput)
        }
        OutputType::Json => {
